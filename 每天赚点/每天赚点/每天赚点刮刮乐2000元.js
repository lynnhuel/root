 ...
 * 
 * ========= 青龙 =========
 * 变量格式：export mtzdgglapp2000y=' xxxx & xxx @  xxxx & xxx '  多个账号用 @分割 
 * 
 */
 ...
 //////////////////////
 let mtzdgglapp2000y = process.env.mtzdgglapp2000y;               // 这里是 从青龙的 配置文件 读取你写的变量
 let mtzdgglapp2000yArr = [];
 let data = '';
 let msg = '';
 ...
 !(async () => {
	 ...
	 if (!(await Envs()))  	//多账号分割 判断变量是否为空  初步处理多账号
		 ...
		 console.log(`\n=================== 共找到 ${mtzdgglapp2000yArr.length} 个账号 ===================`)
 
		 if (debug) {
			 //console.log(`【debug】 这是你的全部账号数组:\n ${mtzdgglapp2000yArr}`);  //这是打印账号信息
		 }
 
 
		 for (let index = 0; index < mtzdgglapp2000yArr.length; index++) {
 
 
			 let num = index + 1
			 console.log(`\n========= 开始【第 ${num} 个账号】=========\n`)
 
			 data = mtzdgglapp2000yArr[index].split('#');      // 这里是分割你每个账号的每个小项   
			 deviceId = mtzdgglapp2000y.match(/device_id=[-\w]{0,100}/);   //获取当前设备的id
...
function 刮刮乐2000元翻倍(timeout = 0) {
 ...
 async function Envs() {
	 if (mtzdgglapp2000y) {
		 if (mtzdgglapp2000y.indexOf("@") != -1) {
			 mtzdgglapp2000y.split("@").forEach((item) => {
				 mtzdgglapp2000yArr.push(item);
			 });
		 } else {
			 mtzdgglapp2000yArr.push(mtzdgglapp2000y);
		 }
	 } else {
		 console.log(`\n 【${$.name}】：未填写变量 mtzdgglapp2000y`)
		 return;
	 }
