 ...
 * 
 * ========= 青龙 =========
 * 变量格式：export mtzdgglapp8000jb=' xxxx & xxx @  xxxx & xxx '  多个账号用 @分割 
 * 
 */
 ...
 //////////////////////
 let mtzdgglapp8000jb = process.env.mtzdgglapp8000jb;               // 这里是 从青龙的 配置文件 读取你写的变量
 let mtzdgglapp8000jbArr = [];
 let data = '';
 let msg = '';
 ...
 !(async () => {
	 ...
	 if (!(await Envs()))  	//多账号分割 判断变量是否为空  初步处理多账号
		 ...
		 console.log(`\n=================== 共找到 ${mtzdgglapp8000jbArr.length} 个账号 ===================`)
 
		 if (debug) {
			 //console.log(`【debug】 这是你的全部账号数组:\n ${mtzdgglapp8000jbArr}`);  //这是打印账号信息
		 }
 
 
		 for (let index = 0; index < mtzdgglapp8000jbArr.length; index++) {
 
 
			 let num = index + 1
			 console.log(`\n========= 开始【第 ${num} 个账号】=========\n`)
 
			 data = mtzdgglapp8000jbArr[index].split('#');      // 这里是分割你每个账号的每个小项   
			 deviceId = mtzdgglapp8000jb.match(/device_id=[-\w]{0,100}/);   //获取当前设备的id
...
function 刮刮乐8000金币翻倍(timeout = 0) {
		...
		$.post(url, async (err, resp, data) => {
			try {
				...
				if (result.success == true) {
					...
					console.log(`【刮刮乐8000金币翻倍】：${result.msg} 🎉获得:${result.obj.gold}金币 \n`)
				} else {

					console.log(`【刮刮乐8000金币翻倍】：${result.msg} 🎉 \n`)
 ...
 async function Envs() {
	 if (mtzdgglapp8000jb) {
		 if (mtzdgglapp8000jb.indexOf("@") != -1) {
			 mtzdgglapp8000jb.split("@").forEach((item) => {
				 mtzdgglapp8000jbArr.push(item);
			 });
		 } else {
			 mtzdgglapp8000jbArr.push(mtzdgglapp8000jb);
		 }
	 } else {
		 console.log(`\n 【${$.name}】：未填写变量 mtzdgglapp8000jb`)
		 return;
	 }
