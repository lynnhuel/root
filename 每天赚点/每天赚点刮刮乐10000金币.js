 ...
 * 
 * ========= 青龙 =========
 * 变量格式：export mtzdgglapp10000jb=' xxxx & xxx @  xxxx & xxx '  多个账号用 @分割 
 * 
 */
 ...
 //////////////////////
 let mtzdgglapp10000jb = process.env.mtzdgglapp10000jb;               // 这里是 从青龙的 配置文件 读取你写的变量
 let mtzdgglapp10000jbArr = [];
 let data = '';
 let msg = '';
 ...
 !(async () => {
	 ...
	 if (!(await Envs()))  	//多账号分割 判断变量是否为空  初步处理多账号
		 ...
		 console.log(`\n=================== 共找到 ${mtzdgglapp10000jbArr.length} 个账号 ===================`)
 
		 if (debug) {
			 //console.log(`【debug】 这是你的全部账号数组:\n ${mtzdgglapp10000jbArr}`);  //这是打印账号信息
		 }
 
 
		 for (let index = 0; index < mtzdgglapp10000jbArr.length; index++) {
 
 
			 let num = index + 1
			 console.log(`\n========= 开始【第 ${num} 个账号】=========\n`)
 
			 data = mtzdgglapp10000jbArr[index].split('#');      // 这里是分割你每个账号的每个小项   
			 deviceId = mtzdgglapp10000jb.match(/device_id=[-\w]{0,100}/);   //获取当前设备的id
...
function 刮刮乐10000金币翻倍(timeout = 0) {
		...
		$.post(url, async (err, resp, data) => {
			try {
				...
				if (result.success == true) {
					...
					console.log(`【刮刮乐10000金币翻倍】：${result.msg} 🎉获得:${result.obj.gold}金币 \n`)
				} else {

					console.log(`【刮刮乐10000金币翻倍】：${result.msg} 🎉 \n`)
 ...
 // ============================================变量检查============================================ \\
 async function Envs() {
	 if (mtzdgglapp10000jb) {
		 if (mtzdgglapp10000jb.indexOf("@") != -1) {
			 mtzdgglapp10000jb.split("@").forEach((item) => {
				 mtzdgglapp10000jbArr.push(item);
			 });
		 } else {
			 mtzdgglapp10000jbArr.push(mtzdgglapp10000jb);
		 }
	 } else {
		 console.log(`\n 【${$.name}】：未填写变量 mtzdgglapp10000jb`)
		 return;
	 }
