 ...
 * 
 * ========= 青龙 =========
 * 变量格式：export mtzdgglapp500y=' xxxx & xxx @  xxxx & xxx '  多个账号用 @分割 
 * 
 */
 ...
 //////////////////////
 let mtzdgglapp500y = process.env.mtzdgglapp500y;               // 这里是 从青龙的 配置文件 读取你写的变量
 let mtzdgglapp500yArr = [];
 let data = '';
 let msg = '';
 ...
 !(async () => {
	 ...
	 if (!(await Envs()))  	//多账号分割 判断变量是否为空  初步处理多账号
		 ...
		 console.log(`\n=================== 共找到 ${mtzdgglapp500yArr.length} 个账号 ===================`)
 
		 if (debug) {
			 //console.log(`【debug】 这是你的全部账号数组:\n ${mtzdgglapp500yArr}`);  //这是打印账号信息
		 }
 
 
		 for (let index = 0; index < mtzdgglapp500yArr.length; index++) {
 
 
			 let num = index + 1
			 console.log(`\n========= 开始【第 ${num} 个账号】=========\n`)
 
			 data = mtzdgglapp500yArr[index].split('#');      // 这里是分割你每个账号的每个小项   
			 deviceId = mtzdgglapp500y.match(/device_id=[-\w]{0,100}/);   //获取当前设备的id
...
function 刮刮乐500元翻倍(timeout = 0) {
 ...
 async function Envs() {
	 if (mtzdgglapp500y) {
		 if (mtzdgglapp500y.indexOf("@") != -1) {
			 mtzdgglapp500y.split("@").forEach((item) => {
				 mtzdgglapp500yArr.push(item);
			 });
		 } else {
			 mtzdgglapp500yArr.push(mtzdgglapp500y);
		 }
	 } else {
		 console.log(`\n 【${$.name}】：未填写变量 mtzdgglapp500y`)
		 return;
	 }
