 ...
 * 
 * ========= 青龙 =========
 * 变量格式：export mtzdgglapp666y=' xxxx & xxx @  xxxx & xxx '  多个账号用 @分割 
 * 
 */
 ...
 //////////////////////
 let mtzdgglapp666y = process.env.mtzdgglapp666y;               // 这里是 从青龙的 配置文件 读取你写的变量
 let mtzdgglapp666yArr = [];
 let data = '';
 let msg = '';
 ...
 !(async () => {
	 ...
	 if (!(await Envs()))  	//多账号分割 判断变量是否为空  初步处理多账号
		 ...
		 console.log(`\n=================== 共找到 ${mtzdgglapp666yArr.length} 个账号 ===================`)
 
		 if (debug) {
			 //console.log(`【debug】 这是你的全部账号数组:\n ${mtzdgglapp666yArr}`);  //这是打印账号信息
		 }
 
 
		 for (let index = 0; index < mtzdgglapp666yArr.length; index++) {
 
 
			 let num = index + 1
			 console.log(`\n========= 开始【第 ${num} 个账号】=========\n`)
 
			 data = mtzdgglapp666yArr[index].split('#');      // 这里是分割你每个账号的每个小项   
			 deviceId = mtzdgglapp666y.match(/device_id=[-\w]{0,100}/);   //获取当前设备的id
...
function 刮刮乐666元翻倍(timeout = 0) {
		...
		$.post(url, async (err, resp, data) => {
			try {
				...
				if (result.success == true) {
					...
					console.log(`【刮刮乐66.6元翻倍】：${result.msg} 🎉获得:${result.obj.gold}金币 \n`)
				} else {

					console.log(`【刮刮乐66.6元翻倍】：${result.msg} 🎉 \n`)
 ...
 async function Envs() {
	 if (mtzdgglapp666y) {
		 if (mtzdgglapp666y.indexOf("@") != -1) {
			 mtzdgglapp666y.split("@").forEach((item) => {
				 mtzdgglapp666yArr.push(item);
			 });
		 } else {
			 mtzdgglapp666yArr.push(mtzdgglapp666y);
		 }
	 } else {
		 console.log(`\n 【${$.name}】：未填写变量 mtzdgglapp666y`)
		 return;
	 }
