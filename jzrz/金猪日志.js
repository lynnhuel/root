...
//////////////////////
let jzrz_data = process.env.jzrz_data;               // 这里是 从青龙的 配置文件 读取你写的变量
let jzrz_dataArr = [];
let data = '';
let msg = '';
...
!(async () => {
	...
	if (!(await Envs()))  	//多账号分割 判断变量是否为空  初步处理多账号
		...
		console.log(`\n=================== 共找到 ${jzrz_dataArr.length} 个账号 ===================`)

		if (debug) {
			//console.log(`【debug】 这是你的全部账号数组:\n ${jzrz_dataArr}`);  //这是打印账号信息
		}


		for (let index = 0; index < jzrz_dataArr.length; index++) {


			let num = index + 1
			console.log(`\n========= 开始【第 ${num} 个账号】=========\n`)

			data = jzrz_dataArr[index].split('&');      // 这里是分割你每个账号的每个小项   
...
async function Envs() {
	if (jzrz_data) {
		if (jzrz_data.indexOf("@") != -1) {
			jzrz_data.split("@").forEach((item) => {
				jzrz_dataArr.push(item);
			});
		} else {
			jzrz_dataArr.push(jzrz_data);
		}
	} else {
		console.log(`\n 【${$.name}】：未填写变量 jzrz_data`)
		return;
	}
