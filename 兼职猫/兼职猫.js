 ...
 * 
 * ========= 青龙 =========
 * 变量格式：export jzmapp=' xxxx & xxx @  xxxx & xxx '  多个账号用 @分割 
 * 
 */
 ...
 //////////////////////
 let jzmapp = process.env.jzmapp;               // 这里是 从青龙的 配置文件 读取你写的变量
 let jzmappArr = [];
 let data = '';
 let msg = '';
 ...
 !(async () => {
	 ...
	 if (!(await Envs()))  	//多账号分割 判断变量是否为空  初步处理多账号
		 ...
		 for (let index = 0; index < jzmappArr.length; index++) {
 
 
			 let num = index + 1
			 console.log(`\n========= 开始【第 ${num} 个账号】=========\n`)
 
			 data = jzmappArr[index].split('#');      // 这里是分割你每个账号的每个小项   
			 deviceId = jzmapp.match(/device_id=[-\w]{0,100}/);   //获取当前设备的id
...
function 偷金币(timeout = 0) {
	return new Promise((resolve) => {

		let url = {
			url: `https://h5.mailejifen.com/energy-ball-produce/get-friend-ball`,
			headers: {
				"Host": "h5.mailejifen.com",
				"Connection": "keep-alive",
				"Content-Length": "30",
				"Accept": "application/json, text/plain, */*",
				"Origin": "https://h5.mailejifen.com",
				"User-Agent": "Mozilla/5.0 (Linux; Android 8.1.0; vivo X21A Build/OPM1.171019.011; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/62.0.3202.84 Mobile Safari/537.36;jianzhimao/8.4.5/848/vivo",
				"Content-Type": "application/x-www-form-urlencoded",
				"Referer": "https://h5.mailejifen.com/dist/",
				"Accept-Encoding": "gzip, deflate",
				"Accept-Language": "zh-CN,en-US;q=0.9",
				"Cookie": "PHPSESSID=5v6opvm741ss0eempfgrcp14ro; CREDITS_appKey=ftrnel0dtretoovl2x2z0wy1; CREDITS_appUser=32950860; SERVERID=6e99c1bcdadd76c900bcb3cf8fe4cee9|1658804062|1658803580",
				"X-Requested-With": "all.parttimeguidesystem",
				
			},
			body: '{friend_id=1&friend_credit_id=${a}}',
		}
		$.post(url, async (err, resp, data) => {
			try {
				...
				if (result.code == 0) {
					...
					console.log(`【刮刮乐30元翻倍】：${result.msg} 🎉获得:${result.obj.gold}金币 \n`)
				} else {

					console.log(`【刮刮乐30元翻倍】：${result.msg} 🎉 \n`)
 ...
 async function Envs() {
	 if (jzmapp) {
		 if (jzmapp.indexOf("@") != -1) {
			 jzmapp.split("@").forEach((item) => {
				 jzmappArr.push(item);
			 });
		 } else {
			 jzmappArr.push(jzmapp);
		 }
	 } else {
		 console.log(`\n 【${$.name}】：未填写变量 jzmapp`)
		 return;
