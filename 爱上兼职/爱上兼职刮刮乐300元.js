 ...
 * 
 * ========= 青龙 =========
 * 变量格式：export asjzgglapp300y=' xxxx & xxx @  xxxx & xxx '  多个账号用 @分割 
 * 
 */
 ...
 //////////////////////
 let asjzgglapp300y = process.env.asjzgglapp300y;               // 这里是 从青龙的 配置文件 读取你写的变量
 let asjzgglapp300yArr = [];
 let data = '';
 let msg = '';
 ...
 !(async () => {
	 ...
	 if (!(await Envs()))  	//多账号分割 判断变量是否为空  初步处理多账号
		 ...
		 console.log(`\n=================== 共找到 ${asjzgglapp300yArr.length} 个账号 ===================`)
 
		 if (debug) {
			 //console.log(`【debug】 这是你的全部账号数组:\n ${asjzgglapp300yArr}`);  //这是打印账号信息
		 }
 
 
		 for (let index = 0; index < asjzgglapp300yArr.length; index++) {
 
 
			 let num = index + 1
			 console.log(`\n========= 开始【第 ${num} 个账号】=========\n`)
 
			 data = asjzgglapp300yArr[index].split('#');      // 这里是分割你每个账号的每个小项   
			 deviceId = asjzgglapp300y.match(/device_id=[-\w]{0,100}/);   //获取当前设备的id
...
function 刮刮乐300元翻倍(timeout = 0) {
 ...
 async function Envs() {
	 if (asjzgglapp300y) {
		 if (asjzgglapp300y.indexOf("@") != -1) {
			 asjzgglapp300y.split("@").forEach((item) => {
				 asjzgglapp300yArr.push(item);
			 });
		 } else {
			 asjzgglapp300yArr.push(asjzgglapp300y);
		 }
	 } else {
		 console.log(`\n 【${$.name}】：未填写变量 asjzgglapp300y`)
		 return;
	 }
