 ...
 * 
 * ========= 青龙 =========
 * 变量格式：export asjzapp=' xxxx & xxx @  xxxx & xxx '  多个账号用 @分割 
 * 
 */
 ...
 //////////////////////
 let asjzapp = process.env.asjzapp;               // 这里是 从青龙的 配置文件 读取你写的变量
 let asjzappArr = [];
 let data = '';
 let msg = '';
 ...
 !(async () => {
	 ...
	 if (!(await Envs()))  	//多账号分割 判断变量是否为空  初步处理多账号
		 ...
		 for (let index = 0; index < asjzappArr.length; index++) {
 
 
			 let num = index + 1
			 console.log(`\n========= 开始【第 ${num} 个账号】=========\n`)
 
			 data = asjzappArr[index].split('#');      // 这里是分割你每个账号的每个小项   
			 deviceId = asjzapp.match(/device_id=[-\w]{0,100}/);   //获取当前设备的id
...
function 签到加倍(timeout = 0) {
 ...
 async function Envs() {
	 if (asjzapp) {
		 if (asjzapp.indexOf("@") != -1) {
			 asjzapp.split("@").forEach((item) => {
				 asjzappArr.push(item);
			 });
		 } else {
			 asjzappArr.push(asjzapp);
		 }
	 } else {
		 console.log(`\n 【${$.name}】：未填写变量 asjzapp`)
		 return;
