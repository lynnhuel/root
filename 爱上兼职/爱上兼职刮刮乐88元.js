 ...
 * 
 * ========= 青龙 =========
 * 变量格式：export asjzgglapp88y=' xxxx & xxx @  xxxx & xxx '  多个账号用 @分割 
 * 
 */
 ...
 //////////////////////
 let asjzgglapp88y = process.env.asjzgglapp88y;               // 这里是 从青龙的 配置文件 读取你写的变量
 let asjzgglapp88yArr = [];
 let data = '';
 let msg = '';
 ...
 !(async () => {
	 ...
	 if (!(await Envs()))  	//多账号分割 判断变量是否为空  初步处理多账号
		 ...
		 console.log(`\n=================== 共找到 ${asjzgglapp88yArr.length} 个账号 ===================`)
 
		 if (debug) {
			 //console.log(`【debug】 这是你的全部账号数组:\n ${asjzgglapp88yArr}`);  //这是打印账号信息
		 }
 
 
		 for (let index = 0; index < asjzgglapp88yArr.length; index++) {
 
 
			 let num = index + 1
			 console.log(`\n========= 开始【第 ${num} 个账号】=========\n`)
 
			 data = asjzgglapp88yArr[index].split('#');      // 这里是分割你每个账号的每个小项   
			 deviceId = asjzgglapp88y.match(/device_id=[-\w]{0,100}/);   //获取当前设备的id
...
function 刮刮乐88元翻倍(timeout = 0) {
		...
		$.post(url, async (err, resp, data) => {
			try {
				...
				if (result.success == true) {
					...
					console.log(`【刮刮乐88元翻倍】：${result.msg} 🎉获得:${result.obj.gold}金币 \n`)
				} else {

					console.log(`【刮刮乐88元翻倍】：${result.msg} 🎉 \n`)
 ...
 async function Envs() {
	 if (asjzgglapp88y) {
		 if (asjzgglapp88y.indexOf("@") != -1) {
			 asjzgglapp88y.split("@").forEach((item) => {
				 asjzgglapp88yArr.push(item);
			 });
		 } else {
			 asjzgglapp88yArr.push(asjzgglapp88y);
		 }
	 } else {
		 console.log(`\n 【${$.name}】：未填写变量 asjzgglapp88y`)
		 return;
	 }
