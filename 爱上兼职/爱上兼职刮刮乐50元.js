 ...
 * 
 * ========= 青龙 =========
 * 变量格式：export asjzgglapp50y=' xxxx & xxx @  xxxx & xxx '  多个账号用 @分割 
 * 
 */
 ...
 //////////////////////
 let asjzgglapp50y = process.env.asjzgglapp50y;               // 这里是 从青龙的 配置文件 读取你写的变量
 let asjzgglapp50yArr = [];
 let data = '';
 let msg = '';
 ...
 !(async () => {
	 ...
	 if (!(await Envs()))  	//多账号分割 判断变量是否为空  初步处理多账号
		 ...
		 console.log(`\n=================== 共找到 ${asjzgglapp50yArr.length} 个账号 ===================`)
 
		 if (debug) {
			 //console.log(`【debug】 这是你的全部账号数组:\n ${asjzgglapp50yArr}`);  //这是打印账号信息
		 }
 
 
		 for (let index = 0; index < asjzgglapp50yArr.length; index++) {
 
 
			 let num = index + 1
			 console.log(`\n========= 开始【第 ${num} 个账号】=========\n`)
 
			 data = asjzgglapp50yArr[index].split('#');      // 这里是分割你每个账号的每个小项   
			 deviceId = asjzgglapp50y.match(/device_id=[-\w]{0,100}/);   //获取当前设备的id
...
function 刮刮乐50元翻倍(timeout = 0) {
		...
		$.post(url, async (err, resp, data) => {
			try {
				...
				if (result.success == true) {
					...
					console.log(`【刮刮乐50元翻倍】：${result.msg} 🎉获得:${result.obj.gold}金币 \n`)
				} else {

					console.log(`【刮刮乐50元翻倍】：${result.msg} 🎉 \n`)
 ...
 async function Envs() {
	 if (asjzgglapp50y) {
		 if (asjzgglapp50y.indexOf("@") != -1) {
			 asjzgglapp50y.split("@").forEach((item) => {
				 asjzgglapp50yArr.push(item);
			 });
		 } else {
			 asjzgglapp50yArr.push(asjzgglapp50y);
		 }
	 } else {
		 console.log(`\n 【${$.name}】：未填写变量 asjzgglapp50y`)
		 return;
	 }
