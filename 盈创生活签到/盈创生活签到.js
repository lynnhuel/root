...
//////////////////////
let ycsh = process.env.ycsh;               // 这里是 从青龙的 配置文件 读取你写的变量
let ycshArr = [];
let data = '';
let msg = '';
...
!(async () => {
	...
	if (!(await Envs()))  	//多账号分割 判断变量是否为空  初步处理多账号
		...
		for (let index = 0; index < ycshArr.length; index++) {


			let num = index + 1
			console.log(`\n========= 开始【第 ${num} 个账号】=========\n`)

			data = ycshArr[index].split('&');      // 这里是分割你每个账号的每个小项   
	...
	return new Promise((resolve) => {
		...
		$.get(url, async (error, response, data) => {     // 这是一个 get 请求 , 如果是 post  记得把这里改了 
			try {
				...
				if (result.code == 1) {        // 这里是根据服务器返回的数据做判断  方便我们知道任务是否完成了

					console.log(`【签到任務】${result.msg} 🎉 `)
					

				} else if (result.code === 0) {    // 这里是根据服务器返回的数据做判断  方便我们知道任务是否完成了

					console.log(`【签到任务】${result.msg} 🎉 `)
					

				}  else {    // 这里是根据服务器返回的数据做判断  方便我们知道任务是否完成了

					console.log(`\n【签到任务】 失败 ❌ 了呢,可能是网络被外星人抓走了!\n `)
