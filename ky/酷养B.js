...
//////////////////////
let ky_data = process.env.ky_data;               // 这里是 从青龙的 配置文件 读取你写的变量
let ky_dataArr = [];
let data = '';
let msg = '';
...
!(async () => {
	...
	if (!(await Envs()))  	//多账号分割 判断变量是否为空  初步处理多账号
		...
		for (let index = 0; index < ky_dataArr.length; index++) {


			let num = index + 1
			console.log(`\n========= 开始【第 ${num} 个账号】=========\n`)

			data = ky_dataArr[index].split('&');      // 这里是分割你每个账号的每个小项   
...
async function Envs() {
	if (ky_data) {
		if (ky_data.indexOf("@") != -1) {
			ky_data.split("@").forEach((item) => {
				ky_dataArr.push(item);
			});
		} else {
			ky_dataArr.push(ky_data);
		}
	} else {
		console.log(`\n 【${$.name}】：未填写变量 ky_data`)
		return;
