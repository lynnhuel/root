...
function 一键领取睡觉奖励(timeout = 0) {
	return new Promise((resolve) => {

		let url = {
			url: `http://rbb.hotbuybuy.com/wangzhuan/earns/api/sleep/fast`,
			headers: {
				"Connection": "close",
				"appid": "xianyu",
				"appCode": "6",
				...
				"userid": data[1],
				"time": "1659656107240",
				"nsSecret1": "9e4c981c8d423d03468ef7d2e034205d",
				"os_version": "27",
				"isEmulator": "0",
				"Content-Length": "0",
		...
		$.post(url, async (err, resp, data) => {
			try {
				...
				if (result.success == true) {
					...
					console.log(`【一键领取睡觉奖励】：${result.message} 🎉\n获得:${result.data}`)
                    id=result.data.id
				} else {

					console.log(`【一键领取睡觉奖励】获得：${result.message} 🎉\n`)
