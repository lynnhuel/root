 ...
 */
 ...
 //////////////////////
 let klbapp = process.env.klbapp;               // 这里是 从青龙的 配置文件 读取你写的变量
 let klbappArr = [];
 let data = '';
 let msg = '';
 let sign = '';
 ...
 !(async () => {
	 ...
	 if (!(await Envs()))  	//多账号分割 判断变量是否为空  初步处理多账号
		 ...
		 for (let index = 0; index < klbappArr.length; index++) {
 
 
			 let num = index + 1
			 console.log(`\n========= 开始【第 ${num} 个账号】=========\n`)
 
			 data = klbappArr[index].split('#');      // 这里是分割你每个账号的每个小项   
			 deviceId = klbapp.match(/device_id=[-\w]{0,100}/);   //获取当前设备的id
...
function 看视频开宝箱(timeout = 0) {
	return new Promise((resolve) => {

		let url = {
			url: `https://klb.diandianapi.com/api/sign/box`,
			headers: {
				"Host": "klb.diandianapi.com",
				"Connection": "Keep-Alive",
				"Accept-Encoding": "gzip",
				...
				"appname": "klb",
				"systemVersion": "android7.0",
				"sign": "4A7B423B6A3DEC0A4701BBABC1BDBFEA",
				"User-Agent": "com.kuailebang.app/1.0.4/huawei",
		...
		$.post(url, async (err, resp, data) => {
			try {
				...
				if (result.code == 200) {
					...
					console.log(`【看视频开宝箱】：${result.msg} 🎉获得:${result.data.get}金币 \n`)
				} else {

					console.log(`【看视频开宝箱】：${result.msg} 🎉`)
...
function 看视频领金币(timeout = 0) {
	return new Promise((resolve) => {
		...
		let url = {
			...
			headers: {
				...
				"appname": "klb",
				"systemVersion": "android7.0",
				"sign": "1C4E5F5D78C706503078CC67BC13BBBA",
				"User-Agent": "com.kuailebang.app/1.0.4/huawei",
		...
		$.post(url, async (err, resp, data) => {
			try {
				...
				if (result.code == 200) {
					...
					console.log(`【看视频领金币】：${result.msg} 🎉获得:${result.data.reward}金币 \n`)
				} else {

					console.log(`【看视频领金币】：${result.msg} 🎉`)
 ...
 async function Envs() {
	 if (klbapp) {
		 if (klbapp.indexOf("@") != -1) {
			 klbapp.split("@").forEach((item) => {
				 klbappArr.push(item);
			 });
		 } else {
			 klbappArr.push(klbapp);
		 }
	 } else {
		 console.log(`\n 【${$.name}】：未填写变量 klbapp`)
		 return;
