 ...
 * 
 * ========= 青龙 =========
 * 变量格式：export ddzapp=' xxxx & xxx @  xxxx & xxx '  多个账号用 @分割 
 * 
 */
 ...
 //////////////////////
 let ddzapp = process.env.ddzapp;               // 这里是 从青龙的 配置文件 读取你写的变量
 let ddzappArr = [];
 let data = '';
 let msg = '';
 ...
 !(async () => {
	 ...
	 if (!(await Envs()))  	//多账号分割 判断变量是否为空  初步处理多账号
		 ...
		 for (let index = 0; index < ddzappArr.length; index++) {
 
 
			 let num = index + 1
			 console.log(`\n========= 开始【第 ${num} 个账号】=========\n`)
 
			 data = ddzappArr[index].split('&');      // 这里是分割你每个账号的每个小项   
			 deviceId = ddzapp.match(/device_id=[-\w]{0,100}/);   //获取当前设备的id
 ...
 function 签到(timeout = 0) {
	return new Promise((resolve) => {
		...
		let url = {
			url: `https://apptran.dandanz.com//webservice/fuli/sign.ashx?t=2&v=4.1.1&deviceid=869498032246775&phonemodel=vivo%20X21A&osversion=8.1.0&simtype=CHN-UNICOM&simid=460011176420655&package=com.lz.xgly&pid=1&manufacture=vivo&imei1=869498032246767&androidid=29f69b7ecfe1cbe9&macaddress=&userid=${data}&rt=0&issimulator=0&hardware=qcom&token=zd8oct886wicu1fcwtredur78per6diwoiaqtaf2&versionname=411&appname=%E8%9B%8B%E8%9B%8B%E8%B5%9A&installtime=1659674797488&imei0=869498032246775&comuid=59690322&launchtimes=1&installduokai=1&oaid=&openMainGameCount=0&firstopen=1659674797488&unixt=1659674921&keycode=e5f66996c46a1dbc100e85c050bb8187`,
			headers: {
				"User-Agent": "Mozilla/5.0 (Linux; Android 8.1.0; vivo X21A Build/OPM1.171019.011; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/62.0.3202.84 Mobile Safari/537.36 DZ(d3ded2d0b560c52ab32ed79ccea90d17)DZ UX(1659674921)UX",
                "Content-Type": "application/x-www-form-urlencoded",
                "Content-Length": "14",
                "Host": "apptran.dandanz.com",
		...
		$.post(url, async (err, resp, data) => {
			try {
				...
				if (result.status == 0) {
					...
					console.log(`【签到】：${result.msg} 🎉获得:${result.rwcoin}金币`)
					id1 = `${result.data.id}`
				} else {
 ...
 async function Envs() {
	 if (ddzapp) {
		 if (ddzapp.indexOf("@") != -1) {
			 ddzapp.split("@").forEach((item) => {
				 ddzappArr.push(item);
			 });
		 } else {
			 ddzappArr.push(ddzapp);
		 }
	 } else {
		 console.log(`\n 【${$.name}】：未填写变量 ddzapp`)
		 return;
