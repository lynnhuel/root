 ...
 * 
 * ========= 青龙 =========
 * 变量格式：export mtzdapp=' xxxx # xxx @  xxxx # xxx '  多个账号用 @分割 
 * 
 */
 ...
 //////////////////////
 let mtzdapp = process.env.mtzdapp;               // 这里是 从青龙的 配置文件 读取你写的变量
 let mtzdappArr = [];
 let data = '';
 let msg = '';
 ...
 !(async () => {
	 ...
	 if (!(await Envs()))  	//多账号分割 判断变量是否为空  初步处理多账号
		 ...
		 for (let index = 0; index < mtzdappArr.length; index++) {
 
 
			 let num = index + 1
			 console.log(`\n========= 开始【第 ${num} 个账号】=========\n`)
 
			 data = mtzdappArr[index].split('#');      // 这里是分割你每个账号的每个小项   
			 deviceId = mtzdapp.match(/device_id=[-\w]{0,100}/);   //获取当前设备的id
 ...
 function 签到(timeout = 0) {
	return new Promise((resolve) => {
		...
		let url = {
			url: `http://api.tutianads.com/goldhome/incomeRecordController.do?saveGameRecord`,
			headers: {
				"client_id": "webApp",
...
function 签到加倍(timeout = 0) {
	return new Promise((resolve) => {
		...
		let url = {
			url: `http://api.tutianads.com/goldhome/veidoRecordController.do?saveSignRecord`,
			headers: {
				"client_id": "webApp",
				"client_secret": "webApp",
				"Authorization": "", 
 ...
 async function Envs() {
	 if (mtzdapp) {
		 if (mtzdapp.indexOf("@") != -1) {
			 mtzdapp.split("@").forEach((item) => {
				 mtzdappArr.push(item);
			 });
		 } else {
			 mtzdappArr.push(mtzdapp);
		 }
	 } else {
		 console.log(`\n 【${$.name}】：未填写变量 mtzdapp`)
		 return;
